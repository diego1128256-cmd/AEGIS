...
*Open-source cybersecurity platform with an 11/11 detection score, sub-300ms pipeline, honeypot deception, and AI-powered autonomous response.*
...
AEGIS is an autonomous cybersecurity defense platform that detects, analyzes, and responds to threats in real-time. It combines a 5-layer detection pipeline (sub-300ms fast path), 122 Sigma correlation rules, AI-powered triage with MITRE ATT&CK mapping, honeypot deception with breadcrumb traps, and a Rust endpoint agent -- all in a single `docker compose up` deployment.

Built for security teams and homelabs that need Wazuh-level detection with AI-powered intelligence, at zero cost.

**Not a toy.** AEGIS runs in production protecting real services, autonomously blocking attackers via iptables firewall integration. Every AI decision is logged to a full audit trail with model used, reasoning, confidence score, and token cost.
...
```bash
git clone https://github.com/aegis-defense/aegis.git
cd aegis
...
Default credentials are created on first boot. The setup wizard at `/setup` walks through initial network discovery and scan configuration.
...
```
    ...
    [Layer 1] Attack Detector Middleware ──────────── runs on EVERY request
         ...
         |     Auto-block after 10 attacks/IP (5min sliding window)
         |
...
- **Breadcrumb Traps** -- Fake `.env` files with trap credentials (AWS keys, DB passwords, API tokens)
- **Breadcrumb Alert Chain** -- Attacker steals creds from honeypot -> tries them on real API -> middleware detects trap indicator -> critical incident + auto-block
- **Template Rotation** -- Decoy pages rotate every 4 hours to avoid fingerprinting
- **Attacker Profiling** -- TTPs, tools, geolocation, threat scoring per IP with MITRE mapping
...
| `nodes` | 8 | Endpoint agent enrollment, heartbeat, events |
| `reports` | 7 | PDF generation, scheduled reports |
| `settings` | 6 | Client config, scan intervals, notifications |
