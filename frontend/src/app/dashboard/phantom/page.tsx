...
function buildInteractionHeatmap(interactions: InteractionItem[]): number[][] {
  // 7 days x 24 hours grid, all zeros
  const grid: number[][] = Array.from({ length: 7 }, () => new Array(24).fill(0));

  interactions.forEach((ix) => {
    const d = new Date(ix.timestamp);
    // day of week: 0=Sun, 1=Mon, ..., 6=Sat → map to Mon=0..Sun=6
  ...
  const attackerColumns = [
    { key: 'source_ip', label: 'Source IP', sortable: true, render: (row: AttackerItem) => <span className="font-mono text-[#22D3EE] text-[13px]">{row.source_ip}</span> },
    {
      ...
      render: (row: AttackerItem) => (
        <span className={cn('capitalize font-medium text-[13px]', sophisticationColors[row.sophistication] || 'text-zinc-400')}>
          {row.sophistication.replace(/_/g, ' ')}
        </span>
      ),
    },
    { key: 'tools_used', label: 'Tools', render: (row: AttackerItem) => <span className="text-[11px] text-zinc-500 font-mono">{(row.tools_used as string[]).join(', ')}</span> },
    {
      key: 'geo_data', label: 'Origin',
      render: (row: AttackerItem) => {
        const geo = row.geo_data as { country: string; city: string } | null;
        return <span className="text-zinc-400 text-[13px]">{geo ? `${geo.city}, ${geo.country}` : 'Unknown'}</span>;
      },
    },
    { key: 'total_interactions', label: 'Interactions', sortable: true, render: (row: AttackerItem) => <span className="font-mono text-white text-[13px]">{row.total_interactions}</span> },
    { key: 'last_seen', label: 'Last Seen', sortable: true, render: (row: AttackerItem) => <span className="text-zinc-500 text-[11px] font-mono">{formatRelativeTime(row.last_seen as string)}</span> },
  ];
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <h1 className="text-[22px] sm:text-[28px] font-bold text-white tracking-tight">Phantom Network</h1>
          <p className="text-sm text-zinc-500 mt-1 hidden sm:block">Honeypot orchestration, attacker profiling, and deception intelligence</p>
        </div>
        <button
      ...
      <div className="bg-[#18181B] border border-white/[0.06] rounded-2xl overflow-hidden">
        <div className="px-4 sm:px-6 py-4 border-b border-white/[0.06]">
          <span className="text-[14px] font-semibold text-white">Interaction Activity</span>
          <p className="text-[12px] text-zinc-500 mt-0.5 hidden sm:block">Honeypot interaction density by hour and day of week</p>
        </div>
        <div className="p-4 sm:p-6 overflow-x-auto">
          {!hasInteractions ? (
            <div className="h-32 flex items-center justify-center">
              <p className="text-zinc-600 text-[13px]">No interaction data yet</p>
            </div>
          ) : (
            <>
              {/* Hour labels */}
              <div className="flex mb-1.5">
                ...
                {HEATMAP_HOURS.map((h) => (
                  <div key={h} className="flex-1 min-w-[18px] text-center text-[9px] text-zinc-700 font-mono">
              ...
              {HEATMAP_DAYS_PHANTOM.map((day, dayIdx) => (
                <div key={day} className="flex items-center mb-[3px]">
                  <div className="w-10 shrink-0 text-[10px] text-zinc-600 font-medium">{day}</div>
      ...
      <div className="flex items-center gap-2 sm:gap-4 border-b border-white/[0.06] overflow-x-auto">
        {[
          ...
          >
            ...
            {t.label}
            <span className="text-[10px] sm:text-[11px] bg-white/[0.05] px-1 sm:px-1.5 py-0.5 rounded-md">{t.count}</span>
          </button>
        ))}
      </div>
      ...
      {tab === 'grid' && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {honeypots.length === 0 ? (
            <div className="col-span-full bg-[#18181B] border border-white/[0.06] rounded-2xl p-12 text-center">
              <Ghost className="w-12 h-12 text-zinc-600 mx-auto mb-3 opacity-50" />
              <p className="text-zinc-500 text-[13px]">No honeypots deployed. Deploy your first decoy to start gathering threat intelligence.</p>
            </div>
          ) : (
            honeypots.map((hp) => (
              <div key={hp.id} className="bg-[#18181B] border border-white/[0.06] rounded-2xl p-5 hover:border-white/[0.1] transition-all duration-200">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[14px] font-semibold text-white">{hp.name}</p>
                      <p className="text-[11px] text-zinc-500 font-mono">{hp.ip_address}:{hp.port}</p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div className="bg-[#09090B] border border-white/[0.06] rounded-xl p-2.5">
                    <p className="text-[10px] text-zinc-600 uppercase tracking-wider font-medium">Interactions</p>
                    <p className="text-lg font-bold text-white font-mono mt-0.5">{hp.interactions_count}</p>
                  </div>
                  <div className="bg-[#09090B] border border-white/[0.06] rounded-xl p-2.5">
                    <p className="text-[10px] text-zinc-600 uppercase tracking-wider font-medium">Last Rotation</p>
                    <p className="text-[11px] text-zinc-400 mt-1.5">{hp.last_rotation ? formatRelativeTime(hp.last_rotation) : 'Never'}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-1.5 text-[11px] font-medium bg-white/[0.05] hover:bg-white/[0.08] border border-white/[0.06] text-zinc-300 rounded-xl py-2 transition-colors disabled:opacity-50"
                  >
                    <RotateCw className={cn('w-3 h-3', hp.status === 'rotating' && 'animate-spin')} />
                    Rotate
                  </button>
                  <button className="flex items-center justify-center p-2 text-zinc-500 hover:text-[#EF4444] bg-white/[0.05] hover:bg-white/[0.08] border border-white/[0.06] rounded-xl transition-colors">
        ...
        <div className="space-y-3">
          {interactions.length === 0 ? (
            <div className="bg-[#18181B] border border-white/[0.06] rounded-2xl p-12 text-center">
              <CommandLineIcon size={48} className="text-zinc-600 mx-auto mb-3 opacity-50" />
              <p className="text-zinc-500 text-[13px]">No interactions captured yet. Deploy honeypots and wait for attackers.</p>
            </div>
          ) : (
            interactions.map((ix) => (
              <div key={ix.id} className="bg-[#18181B] border border-white/[0.06] rounded-2xl p-5 hover:border-white/[0.1] transition-colors">
                ...
                {ix.commands.length > 0 && (
                  <div className="bg-[#09090B] border border-white/[0.06] rounded-xl p-3 mb-2">
                    <p className="text-[10px] text-zinc-600 mb-1.5 font-medium uppercase tracking-wider">Commands Executed</p>
                    <div className="font-mono text-[11px] text-[#22C55E] space-y-0.5">
                      {ix.commands.map((cmd, i) => (
                        <p key={i}><span className="text-zinc-600">$</span> {cmd}</p>
                      ))}
                    </div>
                  </div>
                )}
                {ix.credentials_tried.length > 0 && (
                  <div className="bg-[#09090B] border border-white/[0.06] rounded-xl p-3">
                    <p className="text-[10px] text-zinc-600 mb-1.5 font-medium uppercase tracking-wider">Credentials Attempted</p>
      ...
      <Modal open={showDeployModal} onClose={() => setShowDeployModal(false)} title="Deploy New Honeypot">
        <div className="space-y-4">
          <div>
            <label className="text-[11px] font-medium text-zinc-500 uppercase tracking-wider block mb-1.5">Name</label>
            <input
              type="text"
              value={newName}
          ...
          <div>
            <label className="text-[11px] font-medium text-zinc-500 uppercase tracking-wider block mb-1.5">Type</label>
            <select
              value={newType}
              onChange={(e) => {
          ...
          <div>
            <label className="text-[11px] font-medium text-zinc-500 uppercase tracking-wider block mb-1.5">Port</label>
            <input
              type="number"
              value={newPort}
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button onClick={() => setShowDeployModal(false)} className="px-4 py-2 text-[13px] text-zinc-500 hover:text-white transition-colors rounded-xl">
              Cancel
            </button>
