...
def decode_jwt_token(token: str) -> dict:
    try:
        payload = jwt.decode(
            ...
            options={"require_iss": False},  # Don't reject legacy tokens without iss
        )
        # Validate issuer if present (new tokens always have it)
        if "iss" in payload and payload["iss"] != "aegis":
